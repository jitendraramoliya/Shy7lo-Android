 ...
 */
public class FilterExpandableListView extends ExpandableListView {
    /*
     ...
     * draw the real infoChild views within it's dispatchDraw function. The reason
     ...
     * infoChild items, giving up no chance to do any sort of animation. Instead
     ...
     * 9. This forces the ExpandableListView to refresh all of it's views again.
     * 10.This time when the ListView will not get any of the infoChild views for
     *    the collapsed group.
     */

    @SuppressWarnings("unused")
    private static final String TAG = FilterExpandableListView.class.getSimpleName();
    ...
    public FilterExpandableListView(Context context) {
        super(context);
    }

    public FilterExpandableListView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public FilterExpandableListView(Context context, AttributeSet attrs, int defStyle) {
...
//     * @see ExpandableListView#setAdapter(ExpandableListAdapter)
     */
    public void setAdapter(FilterExpandableAdapter adapter) {
        super.setAdapter(adapter);

        // Make sure that the adapter extends AnimatedExpandableListAdapter
        if(adapter instanceof FilterExpandableAdapter) {
            this.adapter = (FilterExpandableAdapter) adapter;
    ...
    public void setExternalAdapter(FilterExpandableAdapter adapter) {
        super.setAdapter(adapter);
        ...
        if(adapter instanceof FilterExpandableAdapter) {
            ...
            throw new ClassCastException(adapter.toString() + " must implement FilterExpandableListView");
        }
    }
    ...
    public boolean collapseGroupWithAnimation(int groupPos) {
        ...
        // If the first visible view on the screen is a infoChild view AND it's a
        // infoChild of the group we are trying to collapse, then set that
        // as the first infoChild position of the group... see
     ...
     */
    public static abstract class FilterExpandableAdapter extends BaseExpandableListAdapter {
        private SparseArray<GroupInfo> groupInfo = new SparseArray<GroupInfo>();
        private FilterExpandableListView parent;

        private static final int STATE_IDLE = 0;
        private static final int STATE_EXPANDING = 1;
        private static final int STATE_COLLAPSING = 2;

        private void setParent(FilterExpandableListView parent) {
            this.parent = parent;
        }
        ...
        public final View getChildView(final int groupPosition, int childPosition, boolean isLastChild, View convertView, final ViewGroup parent) {
            ...
            if (info.animating) {
                ...
                if (childPosition < info.firstChildPosition) {
                    ...
                    // and the first visible item is the 2nd infoChild of the group
                ...
                // Make measure specs to measure infoChild views
